...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        application, _ = Application.objects.get_or_create(
            name="FireFox", package="org.mozilla.firefox.test"
        )
        application_version, _ = ApplicationVersion.objects.get_or_create(
            application=application, name="Final", code=1
        )
        ...
        for _ in range(REVIEWS_TO_CREATE):
            review = Review.objects.create(
                play_store_review_id=str(uuid.uuid4()),
                author_name=faker.name(),
                application=application,
                application_version=application_version,
                review_text=faker.text(),
                review_rating=random.randint(0, MAX_REVIEW_RATING),
                last_modified=now(),
                review_language="en",
            )
        ...
        self.stdout.write(
            self.style.SUCCESS("Successfully created %s users" % USER_TO_CREATE)
        )
        self.stdout.write(
            self.style.SUCCESS("Successfully created %s reviews" % REVIEWS_TO_CREATE)
        )
