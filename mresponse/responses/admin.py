...
def get_activity_csv(modeladmin, request, qs):
    ...
    responses_declined = (
        responses.annotate_pass_criteria()
        .filter(
...
class ModerationsCountFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (("yes", "3 or more - pass"), ("no", "Less than 3 - fail"))
...
class PositiveToneCountFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        qs = queryset.annotate(
            positive_in_tone_count=Count(
                "moderations", filter=Q(moderations__positive_in_tone=True)
            )
        )
...
class AddressingIssueCountFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (("yes", "2 or more - pass"), ("no", "Less than 2 - fail"))

    def queryset(self, request, queryset):
        qs = queryset.annotate(
            addressing_the_issue_count=Count(
                "moderations", filter=Q(moderations__addressing_the_issue=True)
            )
        )
...
class PersonalCountFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (("yes", "1 or more - pass"), ("no", "Less than 1 - fail"))

    def queryset(self, request, queryset):
        qs = queryset.annotate(
            personal_count=Count("moderations", filter=Q(moderations__personal=True))
        )
...
class ModeratorResponseFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (("yes", "from moderator"), ("no", "not from moderator"))
...
class ResponseResource(resources.ModelResource):
    ...
    class Meta:
        ...
        fields = ("id", "text", "staff_approved", "submitted_to_play_store")
...
class ResponseAdmin(ExportMixin, admin.ModelAdmin):
    ...
    list_filter = (
        ModerationsCountFilter,
        PositiveToneCountFilter,
        AddressingIssueCountFilter,
        PersonalCountFilter,
        ModeratorResponseFilter,
    )
    actions = [staff_approve_responses, get_activity_csv, upload_to_playstore]
