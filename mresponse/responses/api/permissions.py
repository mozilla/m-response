...
class BypassStaffOrCommunityModerationPermissionOnUpdate(BypassStaffOrCommunityModerationPermission):
    message = 'No permission to bypass community or staff moderation.'

    def has_permission(self, request, view):
        if request.method == "PUT":
            return super(BypassStaffOrCommunityModerationPermissionOnUpdate, self).has_permission(request, view)
        else:
            return True
