...
class ResponseAssignedToUser(models.Model):
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL, models.CASCADE, related_name="response_assignment"
    )
    response = models.ForeignKey(
        "Response", models.CASCADE, related_name="moderation_assignments"
    )
...
class Response(models.Model):
    ...
    approved = models.BooleanField(
        default=False,
        help_text=_("If response meets criteria to be submitted to the Play " "Store."),
    )
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, models.PROTECT, related_name="responses"
    )
    ...
    def submit_to_play_store(self):
        ...
        if not self.can_submit_to_play_store():
            msg = "Response {} is not allowed to be submitted to playstore".format(
                self.pk
            )
            raise PlaystoreUploadException(msg)
        ...
        # Make API call.
        reply_to_review(
            self.review.application, self.review.play_store_review_id, self.text
        )
    ...
    def send_upload_notification(self):
        ...
        body = render_to_string(
            "emails/admin_play_store_upload/body.txt", {"response": self}
        )
        subject = render_to_string(
            "emails/admin_play_store_upload/subject.txt", {"response": self}
        )
