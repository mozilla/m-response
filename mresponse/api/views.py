...
def api_root(request, format=None):
    ...
    if request.user.is_authenticated:
        response_dict["me"] = reverse.reverse("my_user", request=request, format=format)
        response_dict["homepage"] = reverse.reverse(
            "homepage", request=request, format=format
        )
    ...
    response_dict["config"] = reverse.reverse("config", request=request, format=format)
...
class Config(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        response_dict["response_guide_book_url"] = (
            ...
            "2PACX-1vQds-2DY62ztTCyaoyVr5myp5ZszUFhAyfGueTz5OF9Gi5UibS9LoMxAF"
            "kkMhlaH133WF6M3bggD8K4/pub"
        )
        response_dict[
            "feedback_url"
