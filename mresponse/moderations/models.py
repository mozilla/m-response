...
class Moderation(models.Model):
    response = models.ForeignKey(
        "responses.Response", models.PROTECT, related_name="moderations"
    )
    moderator = models.ForeignKey(
        settings.AUTH_USER_MODEL, models.PROTECT, related_name="moderations"
    )
    ...
    personal = models.BooleanField(verbose_name=_("is the response personal?"))
...
class Approval(models.Model):
    COMMUNITY = 1
    STAFF = 2
    APPROVAL_TYPE_CHOICES = ((COMMUNITY, _("community")), (STAFF, _("staff")))
    response = models.ForeignKey(
        "responses.Response", models.PROTECT, related_name="approvals"
    )
    approver = models.ForeignKey(
        settings.AUTH_USER_MODEL, models.PROTECT, related_name="+"
    )
    approval_type = models.PositiveSmallIntegerField(choices=APPROVAL_TYPE_CHOICES)
